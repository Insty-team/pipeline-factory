...
> 모든 모듈(챗봇 응답·AI 시뮬·알림톡 발송·답글 자동·카드뉴스·단골 분석·리포트) 데이터의 single source of truth
...
### 1-1. 일별 (Daily KPIs) — 메인 시트, 대시보드 대시보드 핵심
...
| 답글 자동 건수 | int | 답글 자동 워크플로우 로그 | 자동 | |
| **챗봇 응대 건수** | int | 챗봇 응답 Cloudflare Worker | 자동 | |
| **챗봇 → 예약 전환** | int | 챗봇 응답 trigger 매핑 | 자동 + 수동 보정 | |
| **AI 시뮬 시뮬 요청** ⭐ | int | AI 시뮬 worker | 자동 | 일별 시뮬 횟수 |
| **AI 시뮬 → 예약 전환** ⭐ | int | reservation_chip 클릭 매핑 | 자동 + 수동 보정 | |
| **AI 시뮬 사장 컨설팅 도구 사용** ⭐ | int | Streamlit 미니앱 로그 | 자동 | 방문 시 사용 |
...
### 1-2. 손님 (Customer DB) — 단골 분석 분석 source
...
| **시뮬 트리거?** ⭐ | bool | "시뮬·미리보기·어울려" 키워드 | 자동 | AI 시뮬 호출 트리거 |
...
| 답글 자동/수동 | enum | 답글 자동 워크플로우 | 자동 | auto / manual / pending |
| 답글 작성일 | date | 네이버 | 자동 | |
| 사장 OK? (1주차) | bool | 사장 1탭 | 수동 | |

→ 베타 4주 ~30~50건 예상 (백필 10건 + 신규 20~40건).

### 1-6. 알림톡 발송 (알림톡 발송 Detail)
...
### 1-7. 회원권 (단골 분석 Source) — 사장 1회 CSV 업로드
...
## 2. AI 시뮬 지표 대시보드 노출 (대시보드 핵심 섹션)

> **사용자 요청 반영**: 카톡 시뮬 사용 횟수가 대시보드에 명확히 드러나도록.

### 2-1. 대시보드 대시보드 — AI 시뮬 섹션 mockup
...
### 2-3. 진단 리포트 진단 리포트 A — AI 시뮬 추정 효과 명시
...
AI 시술 시뮬레이션 (AI 시뮬, 1인 속눈썹샵 unique selling point):
...
모듈 합산 추정:
  ...
  - 챗봇 응답: +25~50만/월
  - 알림톡 발송 (노쇼 ↓): +30~80만/월
  - 답글 자동 (단골 회복): +20~50만/월
  - 카드뉴스 (메디핑크 인지): +10~20만/월
  → 합계 +100~230만/월 추정
```
...
→ **베타 4주 후 = 신규 손님 채널별 매핑 100% 완료** = commission 협상 시 "AI 시뮬 → 신규 X명" 산정 근거.
...
- [ ] AI 시뮬 시뮬 컬럼 모두 노출 (대시보드 mockup 확인)
...
- **회원권 데이터 부재 시**: 사장이 카톡 또는 CSV 업로드 못 하면 → 단골 분석 분석 정확도 ↓
...
- 회원권 부재 시 단골 분석은 시술 기록 + 후기 데이터로 fallback

---

## 8. 외부 참조

- 챗봇 응답 (응대 데이터 source): `prototypes/modules/chatbot/chatbot-30q.md`
- AI 시뮬 시뮬 (시뮬 데이터 source): `prototypes/modules/sim/m4sim-simulation.md`
- 알림톡 발송 (발송 데이터 source): `prototypes/modules/alimtok/alimtok-templates.md`
- 답글 자동 (답글 데이터 source): `prototypes/modules/reply/reviews-replies.md`
- 단골 분석 (이 sheet의 손님·회원권 시트 활용): `prototypes/modules/ltv-analysis.md` (다음 산출물)
- 대시보드 대시보드 (이 sheet에서 fetch): `prototypes/modules/dashboard/` (다음 산출물)
